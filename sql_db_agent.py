...
path_to_db_file = "db/salary.db"

engine = create_engine(f"sqlite:///{path_to_db_file}")
file_url = "data/salaries_2023.csv"
df = pd.read_csv(file_url)
df.to_sql("salaries_2023", con=engine, if_exists="replace", index=False)
print(f"Database created successfully{df.head()}")
...
db = SQLDatabase.from_uri(f"sqlite:///{path_to_db_file}")
